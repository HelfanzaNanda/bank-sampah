    ...
    <script>
        ...
        function drawDatatable(){
            $("#main-table").DataTable({
                ...
                "ajax": {
                    "url": `${APP_URL}/api/waste/datatables`,
                ...
                "columns": [
                    ...
                    { title : 'Price / Kg', data: 'price', name: 'price', render : (data) => data ? rupiahFormat(data) : '' },
                    {
                        title : 'Status', data: 'status', name: 'status', render : (data) => {
                        if (data == 'AKTIF') {
                    ...
                    },
                    {
                        title : 'Action', data: null, name: 'action', orderable: false, width: '9%', render : (data, type, row, meta) => {
                            ...
                            action += `        <a data-id="${data.id}" class="btn-change-status dropdown-item" href="#">${data.status == 'AKTIF' ? 'Non Aktifan' : 'Aktifan'}</a>`;
        ...
        $(document).on('click', '.btn-change-status', async function (e) {
            e.preventDefault()
            let id = $(this).data('id')
            const isConfirmed = await showPopupConfirmation('Are you sure you want to disable this data?', '')

            if (isConfirmed) {
                const url = `${APP_URL}/api/waste/${id}/status`;
                const method = "PUT";
                const response = await request(url, method);
                if (response.status) {
                    showSuccessAlert(response.message);
        ...
        $(document).on('click', '.btn-create', async function (e) {
            e.preventDefault()
            const url = `${APP_URL}/waste/create`;
            location.href = url;
        })
        ...
        $(document).on('click', '.btn-edit', async function (e) {
            ...
            const url = `${APP_URL}/waste/${id}/edit`;
            location.href = url;
        })
